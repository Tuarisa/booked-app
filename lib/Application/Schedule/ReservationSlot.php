...
class ReservationSlot implements IReservationSlot
{
	...
	{
		return $this->_displayDate->SetTime($this->Begin())->LessThan($date) && $this->_displayDate->SetTime($this->End())->LessThan($date);
	}

	public function IsFutureDate(Date $date)
	{
		return $this->_displayDate->SetTime($this->Begin())->GreaterThan($date);
	}
